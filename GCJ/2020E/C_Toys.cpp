 ...
 * @Desc        :
 * @Url         :
 * @File Name   :C_Toys.cpp
 * @Created Time:2020-10-22 15:23:16
...
void solve() {
    ...
    if (pq.size()) {
        printf("%d %s", n - (int)pq.size(), INDEF);
        return;
    }
    ll mx = cs = sum, md = 0, cd = 0, cval = sum;
    for (int i = 0; i < n; ++i) {
        if (e[i] + r[i] <= cs) {
            cval += e[i];
            pq.push({e[i] + r[i], i});
